#FII TIC: Pulling data from Qualtrics to identify who needs reminders and who needs to be paid
...
#(1) Pulls data from Qualtrics, the survey platform on which we are collecting data for the Trust & Invest Collaborative (TIC) 
#Part B code (this):
...
#(4) It outputs 2 Excel spreadsheets that show who needs a reminder and who needs a payment (and, if they need payment, how much). One of these shows all of the responses to date (with at most one entry per entity_uuid - t combination). The other one shows just the wave and t number that the user specified in partA.



# CLEAN RAW QUALTRICS DATA: DELETE INVALID RESPONSES, UNWANTED VARIABLES  ----
#Take the raw Qualtrics data that was run in Part A. Keep only observations that are not preview and test responses, those that are not missing an entity_uuid, and those that did not say that they got the link from someone other than FII. Later add to this all the test responses(entity_uuid=ania1234, birthyear=1990, etc.)
...
todaysdate <- today()
filenameall <- paste0("TIC_survey_reminders_payments_all_", todaysdate, ".xls")
WriteXLS(final_output, filenameall)
...
filenamefiltered <- paste0("TIC_survey_reminders_payments_wave", wavenum, "_t", tnum, "_", todaysdate, ".xls")
WriteXLS(final_output_filtered, filenamefiltered)
